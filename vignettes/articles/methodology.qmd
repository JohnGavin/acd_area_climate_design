...
# Resolve a path under the package: prefers system.file (installed), falls
# back to a relative path under the project root for dev-mode rendering.
.resolve_pkg_file <- function(...) {
  p <- system.file(..., package = "acd")
  if (nzchar(p)) return(p)
  file.path(...)
}

# Load OSM cross-check tables (Stage 2 outputs, bundled in inst/extdata)
osm_by_district <- utils::read.csv(
  .resolve_pkg_file("extdata", "osm_coverage_by_district.csv"),
  stringsAsFactors = FALSE
)
osm_total <- utils::read.csv(
  .resolve_pkg_file("extdata", "osm_total_summary.csv"),
  stringsAsFactors = FALSE
)

# Compute by-district breakdown directly from joined data (more reliable than
# reading the static analysis/stage_0/by_district.csv from a relative path,
# and the joined data already has every column we need).
by_district <- joined |>
  sf::st_drop_geometry() |>
  dplyr::filter(!is.na(.data$district)) |>
  dplyr::count(.data$district, name = "n") |>
  dplyr::left_join(
    joined |>
      sf::st_drop_geometry() |>
      dplyr::filter(.data$inside_any_zone) |>
      dplyr::count(.data$district, name = "n_in_zone"),
    by = "district"
  ) |>
  dplyr::mutate(
    n_in_zone   = tidyr::replace_na(.data$n_in_zone, 0L),
    pct_in_zone = round(100 * .data$n_in_zone / .data$n, 1)
  ) |>
  dplyr::arrange(dplyr::desc(.data$pct_in_zone))
...
#|   `r by_district$pct_in_zone[by_district$district == "20"]`%
#|   in-zone and district 21 at
#|   `r by_district$pct_in_zone[by_district$district == "21"]`%,
#|   both well above the suburban low of `r min(by_district$pct_in_zone)`%.
#|   Source: computed inline from joined data; spatial join via `join_buildings_zones()`.
